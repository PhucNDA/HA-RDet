...
from mmrotate.models.dense_heads.rotated_rpn_head import RotatedRPNHead
...
class AlignConv(BaseModule):
    def __init__(self,
        ...
        super(AlignConv, self).__init__(init_cfg)
...
class StageAlignRPNHead(RotatedRPNHead):
    ...
    def get(self):
        with torch.no_grad():
            return self.mapping_gt, self.pos_inds

    def _init_layers(self):
        """Init layers of a CascadeRPN stage."""
        self.rpn_conv = AlignConv(self.in_channels, self.feat_channels,
                                  **self.adapt_cfg)
        if self.with_cls:
    ...
    def _region_targets_single(self,
        ...
        # breakpoint()
        gt_hbboxes = obb2xyxy(gt_bboxes, 'le90')
        ...
        # Level Feature filling
        # print('ok')
        with torch.no_grad():
            # mapping_gt=torch.tensor([],device='cuda:0')
            mapping_gt = []
            # if len(sampling_result.pos_inds)>0:
            # mapping_gt = mapping_gt.copy_(torch.tensor(anchors))
            for lv_feat in range(len(anchors)):
                tmp = []
                for anc in range(len(anchors[lv_feat])):
                    tmp.append(-1)
                mapping_gt.append(tmp)
                # mapping_gt=torch.tensor(mapping_gt, device='cuda:0')
        ...
        if len(pos_inds) > 0:
            ...
            with torch.no_grad():
                for id in range(len(pos_inds)):
                    lv, anc = self.rev[pos_inds[id]]
                    # breakpoint()
                    mapping_gt[lv][anc] = sampling_result.pos_assigned_gt_inds[id]
        ...
        # breakpoint()
        ###
        # print('Shape sampling:', sampling_result.pos_assigned_gt_inds.shape)
        # print('Shape positive:', pos_bbox_targets.shape)
        # print('Shape sampling:',sampling_result.pos_assigned_gt_inds)
        # for i in pos_inds:
        #     print(i,gt_bboxes[mapping_gt[i]].shape)
        #     print(i, bbox_targets[i].shape)
        #     print(i, gt_bboxes[mapping_gt[i]])
        #     print(i, bbox_targets[i])
        ###
        # self.mapping_gt=mapping_gt
        # self.pos_inds=pos_inds
        with torch.no_grad():
            rv_pos = [self.rev[x] for x in pos_inds]
        return (labels, label_weights, bbox_targets, bbox_weights, pos_inds,
                neg_inds, mapping_gt, rv_pos)
    ...
    def region_targets(self,
        ...
        # mapping_gt=anchor_list
        # for im in num_imgs:
        #     for feat in anchor_list[im]:
        #         mapping_gt[im][feat]=-1
        ...
        (all_labels, all_label_weights, all_bbox_targets, all_bbox_weights,
         pos_inds_list, neg_inds_list, mapping_gt_list, rv_pos) = multi_apply(
        ...
        with torch.no_grad():
            # breakpoint()
            self.mapping_gt = mapping_gt_list
            self.pos_inds = rv_pos
        # no valid anchors
    ...
    def anchor_offset_hor(self, anchor_list, anchor_strides, featmap_sizes):
    ...
    def anchor_offset(self, anchor_list, anchor_strides, featmap_sizes, mapping_gt, gt_bbox):
        """ Get offset for deformable conv based on anchor shape
        NOTE: currently support deformable kernel_size=3 and dilation=1
        Args:
            anchor_list (list[list[tensor])): [NI, NLVL, NA, 4] list of
        ...
        num_level_anchors = [anchors.size(0) for anchors in anchor_list[0]]
        # rev=[]
        # for lv_feat in range(num_lvls):
        #     for anc in range(len(anchor_list[0][lv_feat])):
        #         rev.append((lv_feat,anc))

        def get_offset(anchors, featmap_size, stride):
            ...
            # x_ctr, y_ctr, w, h= torch.unbind(anchors, dim=1)
            x_t, y_t, x_b, y_b = torch.unbind(anchors, dim=1)
            x_ctr, y_ctr = (x_t+x_b)*0.5, (y_t+y_b)*0.5
            w, h = -x_t+x_b, -y_t+y_b
            x_ctr, y_ctr, w, h = \
                x_ctr / stride, y_ctr / stride, \
                w / stride, h / stride
            dw, dh = w / kernel_size, h / kernel_size
            x, y = dw[:, None] * xx, dh[:, None] * yy
            x_anchor, y_anchor = x_ctr[:, None], y_ctr[:, None]
            # get offset filed
            # print(x_anchor,x_anchor.shape,x_conv,x_conv.shape)
        ...
        def get_offset_theta(anchors, featmap_size, stride, gt_bbox, mapping):
            """Get the offset of oriented"""
            dtype, device = anchors.dtype, anchors.device
            kernel_size = 3
            feat_h, feat_w = featmap_size
            pad = (kernel_size - 1) // 2
            ...
            # get sampling locations of anchors
            # dtype, device = anchors.dtype, gt_bbox.device
            # x_ctr, y_ctr, w, h, a shape anchors
            # x_ctr, y_ctr, w, h = torch.unbind(anchors, dim=1)
            x_t, y_t, x_b, y_b = torch.unbind(anchors, dim=1)
            x_ctr, y_ctr = (x_t+x_b)*0.5, (y_t+y_b)*0.5
            w, h = -x_t+x_b, -y_t+y_b
            with torch.no_grad():
                q, t, e, r, aa = torch.unbind(gt_bbox, dim=1)
                # breakpoint()
                a = torch.empty((len((anchors))), device='cuda:0')
                a = a.copy_(h)

                def convert(num):
                    if num[0] == torch.tensor(-1, device=device):
                        return 0.0
                    return num
                # breakpoint()
                indices_list = None
                if len(mapping) > 0:
                    mapping = torch.tensor(mapping, dtype=dtype, device=device)
                    indices = mapping != torch.tensor(-1,
                                                      dtype=dtype, device=device)
                    indices_list = torch.nonzero(indices, as_tuple=False)
                    a[:] = 0.0
                    if indices_list.nelement() > 0:
                        # breakpoint()
                        indices_list = torch.unique(indices_list[:, 0])
                        a[indices_list] = mapping[indices_list]
                    # for id in range(len(anchors)):
                    #     if mapping[id][0]!=-1:
                    #         a[id]=aa[mapping[id][2]]
                    #     else:
                    #         a[id]=0.0
            ...
            # Ordinary
            xr = sin[:, None] * x + sin[:, None] * y
            yr = sin[:, None] * x + sin[:, None] * y
            x_anchor, y_anchor = x_ctr[:, None]+xr, y_ctr[:, None]+yr
            # Rotated
            if indices_list != None:
                # breakpoint()
                xr = cos[indices_list, None] * x[indices_list] - \
                    sin[indices_list, None] * y[indices_list]
                yr = sin[indices_list, None] * x[indices_list] + \
                    cos[indices_list, None] * y[indices_list]
                x_anchor[indices_list], y_anchor[indices_list] = xr + \
                    x_ctr[indices_list, None], yr + y_ctr[indices_list, None]
        ...
        for i in range(num_imgs):
            ...
            for lvl in range(num_lvls):
                # with torch.no_grad():
                # breakpoint()
                if len(mapping_gt[i]) > 0:
                    offset = get_offset_theta(
                        anchor_list[i][lvl], featmap_sizes[lvl], anchor_strides[lvl], gt_bbox[i], mapping_gt[i][lvl])
                else:
                    offset = get_offset_theta(
                        anchor_list[i][lvl], featmap_sizes[lvl], anchor_strides[lvl], gt_bbox[i], mapping_gt[i])
                mlvl_offset.append(offset)
            offset_list.append(torch.cat(mlvl_offset))  # [totalNA, 2*ks**2]
        offset_list = images_to_levels(offset_list, num_level_anchors)
        return offset_list
...
class HybridAnchorRPNHead(BaseDenseHead):
        ...
        for i in range(self.num_stages):
            ...
            if stage.adapt_cfg['type'] == 'offset':
                offset_list = stage.anchor_offset(anchor_list,
                                                  ...
                                                  featmap_sizes, mapping_gt, gt_bboxes)
            else:
                offset_list = None
            # breakpoint()
            ...
            with torch.no_grad():
                if i == 0:
                    ###
                    # breakpoint()
                    mapping_gt, pos_inds = stage.get()

                    # mapping=torch.tensor(mapping_gt)
                    # anchor=torch.tensor(anchor_list)

                    # print(mapping.shape)
                    # print(anchor.shape)
                    # print(i, len(mapping_gt),len(anchor_list),len(gt_bboxes))
                    # print(anchor_list)
    ...
    def forward(self, x, img_metas=None):
        ...
        for i in range(self.num_stages):
            stage = self.stages[i]
            if stage.adapt_cfg['type'] == 'offset':
                offset_list = stage.anchor_offset_hor(anchor_list,
                                                      stage.anchor_strides,
                                                      featmap_sizes)
            else:
                offset_list = None
