...
from mmdet.core import (RegionAssigner, images_to_levels, multi_apply)
# from mmrotate.core.bbox.assigners.maxx_assigner import MaxxIoUAssigner
from mmrotate.core import build_assigner, build_sampler
from mmdet.core.utils import select_single_mlvl
from mmrotate.core import obb2xyxy, obb2hbb 
...
from mmrotate.core import rotated_anchor_inside_flags
...
class StageAlignmentRPNHead(OrientedRPNHead):
    def __init__(self,
                 in_channels,
        ...
        super(StageAlignmentRPNHead, self).__init__(
            in_channels,
    ...
    def _region_targets_single(self,
        ...
        tmp_anchors=[]
        for _ in anchors:
            tmp_anchors.append(obb2xyxy(_,'le135'))
        tmp_gt_bboxes=obb2xyxy(gt_bboxes,'le135')
    ...
    def region_targets(self,
                       ...
                       label_channels=1,
                       unmap_outputs=True):
        """See :func:`StageAlignmentRPNHead.get_targets`."""
        ...
        #print('Region:_pos:',num_total_pos,'Region_neg:',num_total_neg)
    ...
    def _get_targets_single(self,
        ...
        assign_result = self.assigner.assign(
            tmp_anchors, tmp_gt_bboxes, gt_bboxes_ignore,
            None if self.sampling else gt_labels)
        ...
        #print('stage:_pos:',len(pos_inds),'stage_neg:',len(neg_inds))
    ...
    def get_targets(self,
        ...
        if isinstance(self.assigner, RegionAssigner):
            ...
            cls_reg_targets = super(StageAlignmentRPNHead, self).get_targets(
                anchor_list,
                valid_flag_list,
                gt_bboxes,
                img_metas,
    ...
    def anchor_offset(self, anchor_list, anchor_strides, featmap_sizes):
        ...
        for i in range(num_imgs):
            ...
            for lvl in range(num_lvls):
                # print(anchor_list[i][lvl], anchor_list[i][lvl].shape)
                offset=get_offset(anchor_list[i][lvl],featmap_sizes[lvl],anchor_strides[lvl])
                # offset = torch.stack([offset_y, offset_x], dim=-1)
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        # regression loss
        # print('okokok:',bbox_targets)
        bbox_targets = bbox_targets.reshape(-1, 5)
        bbox_weights = bbox_weights.reshape(-1, 5)
        bbox_pred = bbox_pred.permute(0, 2, 3, 1).reshape(-1, 5)
        # print(bbox_pred.shape, anchors.reshape(-1,5).shape)
        if self.reg_decoded_bbox:
            anchors = anchors.reshape(-1, 5)
            # print(anchors)
            # print(bbox_pred)
            # print(anchors.shape,bbox_pred.shape)
    ...
    def _get_bboxes_single(self,
        ...
        if proposals.numel() > 0:
            ...
            dets, _ = batched_nms(hproposals, scores, ids, cfg.nms)
        else:
    ...
    #     # assert num_levels == len(bbox_pred)
    #     num_imgs = bbox_pred[0].size(0)
    #     for i in range(num_levels):
    #         assert num_imgs == bbox_pred[i].size(0) == bbox_preds[i].size(0)

    #     device = bbox_pred[0].device
    #     featmap_sizes = [bbox_pred[i].shape[-2:] for i in range(num_levels)]
...
class AlignmentRPNHead(BaseDenseHead):

    def __init__(self, num_stages, stages, train_cfg, test_cfg, init_cfg=None):
        super(AlignmentRPNHead, self).__init__(init_cfg)
    ...
    def loss(self):
        """loss() is implemented in StageAlignmentRPNHead."""
        pass

    def get_bboxes(self):
        """get_bboxes() is implemented in StageAlignmentRPNHead."""
        pass
    ...
    def aug_test_rpn(self, x, img_metas):
        ...
        raise NotImplementedError(
            'AlignmentRPNHead does not support test-time augmentation')